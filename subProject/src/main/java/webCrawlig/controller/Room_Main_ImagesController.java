...
// http://localhost:8090/test/test/images_room


@Controller
public class Room_Main_ImagesController {

	
	private final CrawligService service;

	@Autowired
	public Room_Main_ImagesController(CrawligService service) {
		this.service = service;
	}
	
	
	@GetMapping("/test/test/images_room")
	public void execute() throws IOException {
		...
		int numPlus =0;
			...
			for(int i=0; i<urlList.length; i++) {
		      ...
		      Elements e1 = doc.getElementsByAttributeValue("class","item on");
				...
				for(int k=1; k<e1.select("img").eachAttr("alt").size(); k++) {
				
					
						
						Room_num.add(num_Room);
					
				
					
				
					if(!(img_Name.get(k-1).equals(img_Name.get(k)))){
						num_Room = num_Room+1;
				...
				for(int a=1; a<e1.select("img").eachAttr("alt").size(); a++) { 
					
					if(a==e1.select("img").eachAttr("alt").size()-1) {
		      ...
		      for(int j=0; j<e1.select("img").eachAttr("alt").size(); j++) {
		    	  	...
		    	  	//객실 이름
		    	  	String filename = img_Name.get(j);
		  		    	  
		    	  	
		    	  			
		    	  	
		    	 	//호텔 사진 경로 
		    	  	String filepath = img_Room.get(j);
		    	  	
		    	  	//호텔 우리가 저장할 경로
		    	  	
					/*
					 * String filepathTrue = "hotel_Num"+(i+1)+"/"+ img_Room.get(j)
					 * .substring(filepath.length()-17,filepath.length());
					 */
		    	 
		    	  	                                                     // 룸      호텔 
		    	  	Room_ImagesDTO room_ImagesDTO = new Room_ImagesDTO(room_code, num, filename, filepath);
		    	  	
		    	  	service.roomImagesProcess(room_ImagesDTO);
	...
	static void saveFile(String no, String imgSrc) throws IOException {
		...
		File file = new File("D:\\room_images",no);
