...
public class TestBoardController {
	...
	public String homeLogin(Model model) {
		model.addAttribute("banner", service.indexBannerProcess());
		
		model.addAttribute("rawnumlist", service.RawnumBoardProcess());
	...
	@GetMapping("/room")
	public String mainPage5(SearchDTO searchDTO, Model model) {
		...
		if (searchDTO.getCheck() != null && searchDTO.getCheck().equals("2")) {

			List<String> bedTypeInEnglish = searchDTO.getBedType();
			Map<String, String> translationMap = new HashMap<>();
			if (bedTypeInEnglish != null) {
				translationMap.put("single", "싱글베드 1개");
				translationMap.put("double", "더블베드");
				translationMap.put("twin", "싱글베드 2개");

				List<String> bedTypeInKorean = bedTypeInEnglish.stream().map(translationMap::get)
	...
	public String mainPage4(SearchDTO searchDTO, Model model) {
		...
		List<List<Hotel_Detail_InfoDTO>> hotel_infoList = hotel_detail.stream()
				.map(n -> n.getHotel_detail_InfoDTO())
				.collect(Collectors.toList());
	...
	@PostMapping("/payment")
	public String payMent(@ModelAttribute SearchDTO searchDTO, @ModelAttribute BookingDTO bookingDTO, HttpServletResponse response) {
