...
// http://localhost:8090/test/test/room_detail_address

@Controller
public class hotel_detail_addressController {

	private final CrawligService service;

	@Autowired
	public hotel_detail_addressController(CrawligService service) {
		this.service = service;
	}
	
	
	@GetMapping("/test/test/room_detail_address")
	public void execute() throws IOException {
		...
		/////////////////////////////////////
			...
			for(int i=0; i<urlList.length; i++) {
		      ...
		      List<String> hotel_D_address =e1.select("p").eachText();
		      ...
		      for(int j=0; j<1; j++) {
		    	  ...
		    	  String hotel_detail_address = hotel_D_address.get(j);
		    	  
		    	  Hotel_Detail_AddressDTO H_D_A_DTO = new Hotel_Detail_AddressDTO(num,hotel_detail_address);
		    	  
		    	  service.hotelDetailAddressProcess(H_D_A_DTO);
