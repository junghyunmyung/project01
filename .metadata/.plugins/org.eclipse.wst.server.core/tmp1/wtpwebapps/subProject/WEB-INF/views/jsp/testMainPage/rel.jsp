...
<link type="text/css" rel="stylesheet" href="/resources/testMainPage/css/rel.css" />
...
<script defer type="text/javascript" src='/resources/testMainPage/js/crescendo.js'></script>
		...
		<div class="sliderWrapper controls">
			<div class="control_slider">
				<c:forEach items="${dto.imageDTO}" var="image" varStatus="status">
					<img src="https:${image.filepath}" alt="${image.filename}" width="500" height="320" />
				</c:forEach>
			</div>
...
<footer>
	<div id="btnWrap">
		<button onclick="location.href = 'rel?hotel_code=${searchDTO.hotel_code}&checkin=${searchDTO.checkin}&checkout=${searchDTO.checkout}'" class="on" data-id="container">객실안내/예약</button>
		...
		<div class="room">
			<div class="img-wrap">
				<c:forEach items="${dto.imageDTO}" var="image" varStatus="status">
					<c:if test="${status.first}">
						<img class="lazy" data-num="${roomStatus.count}" src="https:${image.filepath}" alt="${image.filename}" width="376" height="226">
					</c:if>
				</c:forEach>
			</div>
			...
			<div class="room-text">
				...
				<div class="info-wrap">
					...
					<c:choose>
						<c:when test="${dto.totalPrice != null}">
							<button onclick="location.href = 'pay?hotel_code=${searchDTO.hotel_code}&checkin=${searchDTO.checkin}&checkout=${searchDTO.checkout}&room_code=${dto.room_code}'">예약 | ${dto.totalPrice} | ${dto.days} (박)</button>
						</c:when>
						<c:otherwise>
							<button onclick="location.href = 'pay?hotel_code=${searchDTO.hotel_code}&checkin=${searchDTO.checkin}&checkout=${searchDTO.checkout}&room_code=${dto.room_code}'">예약</button>
						</c:otherwise>
					</c:choose>
				</div>
			</div>
		...
		<article id="reviewTap" class="reviewTap">
			...
			<div id="reviewFormContainer" style="display:none;">
				<form id="reviewFrm" method="post" enctype="multipart/form-data">
					<input type="hidden" id="hotel_code" name="hotel_code" value="${searchDTO.hotel_code}"/>
					<div>작성자</div>
					...
					<div>rating
						...
						<div class="rating-clickable"></div>
					</div>
					<div><input multiple="multiple" type="file" id="images" name="images" /></div>
			...
			<div id="commentFormContainer" style="display:none;">
				<form id="commentFrm">
					<textarea id="business_comment" name="business_comment" rows="5"></textarea>
					<input type="hidden" id="current_review_code" name="current_review_code" value="">
					<div><input type="button" id="submitComment" value="작성" />
					<input type="button" id="cancelComment" value="취소" /></div>
				</form>
			</div>
